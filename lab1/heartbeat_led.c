...
int main()
{
    ...
    */
    ...
    while (1)
    {
        // Brightness function!
        /*
        PWM Generation that will make a pulse 20 times while fading away
        1 - 100 with steps of 5 = 20 times
        From and To values are divided by that step
        */
        for (int i = 0; i <= 100; i += 5)
        {
            if (i == 0)
                i = 1; // To avoid divide by zero
            pwm_gen(0, 100 / ((float)i), 0.1, total_time);
            pwm_gen(100 / ((float)i), 0, 0.4, total_time);
            pwm_gen(0, 50 / ((float)i), 0.1, total_time);
            pwm_gen(50 / ((float)i), 0, 0.4, total_time);
            OCR1A = (total_time);
            delay_wrapper(2000);
        }
        delay_wrapper(2000);
